    ...
    for (final foodId in foodIds) {
      try {
        ...
        if (foodResponse.statusCode != 200) {
          debugPrint(
              '❌ Failed to fetch foodId $foodId. Status: ${foodResponse.statusCode}');
          continue;
        }
        final foodData = jsonDecode(foodResponse.body)['food'];
        if (foodData == null ||
            foodData['servings'] == null ||
            foodData['servings']['serving'] == null) {
          debugPrint('❌ Servings data missing for foodId: $foodId');
          continue;
        }
        ...
        final carbG =
            double.tryParse(servingData['carbohydrate'] ?? '0') ?? 0;
        ...
        data.add(FoodDetailsModel(
          ...
          totalFat: double.tryParse(
              servingData['total_fat'] ?? servingData['fat'] ?? '0'),
          saturatedFat: double.tryParse(servingData['saturated_fat'] ?? '0'),
          transFat: double.tryParse(servingData['trans_fat'] ?? '0'),
          polyunsaturatedFat:
              double.tryParse(servingData['polyunsaturated_fat'] ?? '0'),
          monounsaturatedFat:
              double.tryParse(servingData['monounsaturated_fat'] ?? '0'),
          ...
          totalCarbohydrate: double.tryParse(
              servingData['total_carbohydrate'] ??
                  servingData['carbohydrate'] ??
                  '0'),
          ...
          totalProtein: double.tryParse(
              servingData['total_protein'] ?? servingData['protein'] ?? '0'),
        ...
        debugPrint('❌ Error processing foodId $foodId: $e');
        continue;
      }
    }
    return data;
  } catch (e) {
    debugPrint('❌ A critical error occurred in fetchFavoriteFoodsFromFirebase: $e');
    return []; // Return an empty list on any critical failure
  }
}
